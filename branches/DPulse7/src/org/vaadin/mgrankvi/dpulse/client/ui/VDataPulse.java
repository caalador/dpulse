...
public class VDataPulse extends Widget implements Paintable, ClickHandler {
	...
	public void updateFromUIDL(final UIDL uidl, final ApplicationConnection client) {
		...
		setWidth(options.getIntAttribute("canvaswidth") + "px");
		setHeight(options.getIntAttribute("canvasheight") + "px");
		canvas.setCoordinateSpaceWidth(options.getIntAttribute("canvaswidth"));
		canvas.setCoordinateSpaceHeight(options.getIntAttribute("canvasheight"));
	...
	public void onClick(final ClickEvent event) {
		...
		for (final Entry<Integer, Selectable> set : objects.entrySet()) {
			if (set.getValue().pointInObject(x, y)) {
				client.updateVariable(paintableId, CLICK_EVENT, set.getKey(), true);
				break;
			}
		}
	}
	...
	class Poller extends Timer {
		...
		@Override
		public void run() {
			if (!client.hasActiveRequest() && Util.isAttachedAndDisplayed(VDataPulse.this)) {
				// client.sendPendingVariableChanges();
				client.updateVariable(paintableId, "refresh", "update", true);
			}
		}
