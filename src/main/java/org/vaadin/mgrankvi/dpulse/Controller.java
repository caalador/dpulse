...
public class Controller extends HorizontalLayout {
	...
	private void init() {
		...
		pollTime.addValueChangeListener(timingChange);
		...
		addComponent(new Button("Clear connectors", new Button.ClickListener() {
			...
			public void buttonClick(final ClickEvent event) {
				...
				connected.markAsDirty();// requestRepaint();
			}
		}));
	}
	...
	private void addConnector(final int i) {
		...
		case ECHO:
			connected.addConnection(new EchoConnector(target.getValue(), "Random result set N-" + i));
			break;
		case HTML:
			connected.addConnection(new HtmlConnector(target.getValue(), "Random result set N-" + i));
			break;
		case REACHABLE:
			connected.addConnection(new IsReachableConnector(target.getValue(), "Random result set N-" + i));
			break;
		default:
			connected.addConnection(new HtmlConnector(target.getValue(), "Random result set N-" + i));
		}
